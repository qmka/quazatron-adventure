...
const gameController = (function () {
    ...
    const makeLocation = (g) => {
        ...
        if (g.currentLoc === 17 && !g.flags.isPortcullisOpened) description += " Решётка опущена - не пройти.";
        // Если люк открыт
        if (g.currentLoc === 18 && g.flags.isTrapdoorOpened) description += " В полу комнаты дыра, через которую можно спуститься вниз.";
        // Если люк закрыт
        if (g.currentLoc === 18 && !g.flags.isTrapdoorOpened) description += " В полу есть закрытый люк.";
...
const controller = (function (gameCtrl, wordCtrl) {
    ...
    let gameDataVar = {
        ...
        locations: [{
            ...
            index: 20,
            desc: "Вы в жутко холодной комнате. Всё здесь покрыто толстым слоем льда. Выходы на востоке и на севере.",
            dir: [-1, -1, -1, 19, -1, -1]
        }, {
            index: 21,
            ...
            index: 25,
            desc: "Вы в комнате, которая когда-то была помещением для охраны. На одной из стен вы можете видеть чудом сохранившийся посреди всей этой разрухи рычаг.",
            dir: [-1, 26, 20, -1, -1, -1]
        }, {
            index: 26,
            ...
            index: 27,
            desc: "Вы в роскошно обставленном зале, обстановка которого резко контрастирует с разрухой в других комнатах.",
            dir: [-1, -1, 17, -1, -1, -1]
        }, {
            index: 28,
            desc: "Вы в маленькой комнате. Сквозь разбитое окно в потолке дует холодный ветер. На кровати в углу спит принцесса, а воздух вокруг наэлектризован от чар.",
            dir: [-1, 27, -1, -1, -1, -1]
        }],
